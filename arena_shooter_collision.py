...
from geometry.box import TexturedBoxGeometry
...
class ArenaShooterCollision(Base):
    ...
    def __init__(self):
        ...
        # Timer
        self.total_time = 200  # 3 minutos em segundos
        ...
        self.has_guitar = False  # Começar sem a guitarra
        self.guitar_pickup_time = None
        self.guitar_drop_time = None
        
        # Adicionar variável de estado para apanhar o French Horn
        self.has_french_horn = False
        self.french_horn_pickup_time = None
        self.french_horn_drop_time = None
        ...
        # Ajustar a câmera para a resolução do monitor
        self.camera = Camera(aspect_ratio=screen_size[0]/screen_size[1])
        ...
        self.tuba_light = PointLight(
            color=[0.0, 2.0, 8.0],
            position=[0, 0.5, 0],  # Posição inicial ajustada
            attenuation=[1.0, 0.05, 0.005]
        )
        self.scene.add(self.tuba_light)

        # Carregar textura do chão
        self.floor_texture = Texture(file_name="images/darkbricks.png")
        ...
        # Criar o ambiente
        self.create_environment(floor_material)
        ...
        self.tuba_appear_time = 60.0  # Tempo em segundos (60 segundos)
        self.tuba_has_appeared = False
        self.tuba_message_time = None
        self.tuba_message_duration = 5.0  # Duração da mensagem em segundos

        # Adicionar variável para tempo de aparecimento do French Horn
        self.french_horn_appear_time = 100.0  # Tempo em segundos (1 minuto e 40 segundos)
        self.french_horn_has_appeared = False
        self.french_horn_message_time = None
        self.french_horn_message_duration = 5.0  # Duração da mensagem em segundos
        ...
        french_horn_geometry = TubaGeometry2(1, 1.5, 1, positions, uvs)
        ...
        self.french_horn_mesh.visible = False  # Começar invisível
        self.scene.add(self.french_horn_mesh)

        # Adicionar variável para a posição inicial do French Horn
        self.french_horn_initial_position = [0, self.initial_height, 0]
    ...
    def setup_hud(self):
        ...
        self.guitar_weapon_mesh.visible = False  # Começar invisível pois o jogador não tem a guitarra
        self.camera.add(self.guitar_weapon_mesh)

        # No setup_hud, criar mesh do French Horn para a "arma":
        positions, uvs = my_obj_reader2("french-horn/source/frenchhorn obj/frenchhorn OBJ 2/french horn 2.obj")
        french_horn_weapon_geometry = TubaGeometry2(1, 1.5, 1, positions, uvs)
        french_horn_weapon_texture = Texture(file_name="french-horn/source/frenchhorn obj/frenchhorn OBJ 2/dif map.jpg")
        french_horn_weapon_material = PhongMaterial(
            texture=french_horn_weapon_texture,
            property_dict={
                "baseColor": [1.0, 1.0, 1.0],
                "specularStrength": 1.0,
                "shininess": 16.0
        ...
        self.french_horn_weapon_mesh = Mesh(french_horn_weapon_geometry, french_horn_weapon_material)
        self.french_horn_weapon_mesh.scale(0.02)  # Reduzido de 0.2 para 0.1
        self.french_horn_weapon_mesh.set_position([0.13, -0.18, -0.5])
        self.french_horn_weapon_mesh.rotate_x(math.pi)  # Orientação correta
        self.french_horn_weapon_mesh.rotate_y(math.pi/2)  # Rotação adicional para corrigir a orientação
        self.french_horn_weapon_mesh.visible = False
        self.camera.add(self.french_horn_weapon_mesh)

        # No setup_hud, adicionar criação de um texto/ícone para o French Horn:
        self.french_horn_icon_surface = pygame.Surface((180, 30))
        self.french_horn_icon_surface.set_colorkey((0,0,0))
        self.french_horn_icon_font = pygame.font.Font(None, 24)
        self.french_horn_icon_texture = Texture()
        self.french_horn_icon_material = TextureMaterial(texture=self.french_horn_icon_texture, property_dict={"doubleSide": True})
        french_horn_icon_geometry = TexturedPlaneGeometry(width=0.25, height=0.04)
        self.french_horn_icon_mesh = Mesh(french_horn_icon_geometry, self.french_horn_icon_material)
        self.french_horn_icon_mesh.set_position([-0.7, -0.35, -0.5])  # canto inferior esquerdo, acima do ícone da guitarra
        self.camera.add(self.french_horn_icon_mesh)
        ...
        self.instrument_message_surface = pygame.Surface((400, 50))
        ...
        self.instrument_message_material = TextureMaterial(texture=self.instrument_message_texture, property_dict={"doubleSide": True})
        instrument_message_geometry = TexturedPlaneGeometry(width=0.5, height=0.1)
        self.instrument_message_mesh = Mesh(instrument_message_geometry, self.instrument_message_material)
        self.instrument_message_mesh.set_position([0, 0, -0.3])  # Ajustado para -0.3 para ficar à frente da mira
        self.instrument_message_mesh.visible = False
        self.camera.add(self.instrument_message_mesh)

    def create_environment(self, floor_material):
        # Criar skybox
        sky_texture = Texture(file_name="images/hellsky.jpg")
        ...
        sky_material = TextureMaterial(
            texture=sky_texture,
            property_dict={
                "baseColor": [0.6, 0.6, 0.6],  # Céu mais escuro
                "doubleSide": True
            }
        )
        ...
        positions, uvs = my_obj_reader2("geometry/Skull.obj")
        character_geometry = TubaGeometry2(1, 1, 1, positions, uvs)
        character_texture = Texture(file_name="images/skull.png")
        ...
        # Lista de coordenadas para os personagens - mais longe do centro
        self.character_positions = [
            [-25.0, 2.0, -40.0],  # Aumentado Y de 0.5 para 1.0
            [-20.0, 2.0, -50.0],  # Aumentado Y de 0.5 para 1.0
            [25.0, 2.0, -45.0],   # Aumentado Y de 0.5 para 1.0
            [30.0, 2.0, -30.0],   # Aumentado Y de 0.5 para 1.0
            [20.0, 2.0, -60.0]    # Aumentado Y de 0.5 para 1.0
        ]
        ...
        for pos in self.character_positions:
            ...
            character_mesh.scale(2)  # Mantendo a escala pequena
            character_mesh.rotate_y(math.radians(90))  # Virar para frente
            self.scene.add(character_mesh)
            self.character_meshes.append(character_mesh)

        # Adicionar o modelo do Coliseu
        positions, uvs = my_obj_reader2("geometry/colosseum.obj")
        colosseum_geometry = ColosseumGeometry(1, 1, 1, positions, uvs)
        # Material do Coliseu sem textura, só cinzento
        colosseum_material = PhongMaterial(
            property_dict={
                "baseColor": [0.0, 0.0, 0.0],  # Preto
                "specularStrength": 0.3,
                "shininess": 4.0
            },
            number_of_light_sources=3
        )
        # Criar e posicionar o Coliseu
        self.colosseum_mesh = Mesh(colosseum_geometry, colosseum_material)
        self.colosseum_mesh.set_position([0, 0, -20])  # Posição Y ajustada para alinhar com o chão
        self.colosseum_mesh.scale(15.0)  # Aumentado para 3x o tamanho original (era 5.0)
        self.scene.add(self.colosseum_mesh)
        ...
        self.guitar_mesh.visible = True  # Começar visível para ser apanhada
        self.scene.add(self.guitar_mesh)

        # Criar chão
        floor_size = 3000  # Aumentado para 3x o tamanho original
        floor = Mesh(
            TexturedPlaneGeometry(
                width=floor_size,
                height=floor_size,
                repeat_u=600,  # Aumentado proporcionalmente
                repeat_v=600   # Aumentado proporcionalmente
            ),
            floor_material
        )
    ...
    def check_wall_collision(self, position):
        """
        Verifica se há colisão entre o jogador e a parede e retorna informações sobre a colisão
        Returns: (colidiu, posição_ajustada)
        """
        # Margem de colisão (raio do "cilindro" do jogador)
        player_radius = 0.5
        # Remover colisão com a parede vermelha
    ...
    def create_bullet(self):
        ...
        elif self.has_french_horn:
            # Usar o som do trombone para o French Horn
            current_time = pygame.time.get_ticks() / 1000
            if current_time - self.last_guitar_shot_time < self.guitar_shot_cooldown:
                return  # Não disparar se ainda estiver em cooldown
                
            self.last_guitar_shot_time = current_time
            self.base_sound.play()  # Tocar o som do trombone
            
            # Obter a posição da câmara
            camera_position = self.camera.global_position
            # Obter a direção da câmera usando a matriz de rotação global
            camera_matrix = self.camera.global_matrix
            base_direction = [-camera_matrix[0][2],
                            -camera_matrix[1][2],
                            -camera_matrix[2][2]]
            
            # Normalizar o vetor de direção base
            length = math.sqrt(sum(d*d for d in base_direction))
            base_direction = [d/length for d in base_direction]
            
            # Criar 5 notas com direções aleatórias
            for _ in range(5):
                # Criar material com cor aleatória
                random_color = [random.random(), random.random(), random.random()]
                bullet_material = TextureMaterial(
                ...
                # Fazer a bala sair do French Horn (ajustar offsets conforme necessário)
                offset = 0.8  # distância à frente
                offset_x = 0.10  # desvio lateral (direita)
                offset_y = -0.08  # desvio vertical (baixo)
                right = [self.camera.global_matrix[0][0], self.camera.global_matrix[1][0], self.camera.global_matrix[2][0]]
                up = [self.camera.global_matrix[0][1], self.camera.global_matrix[1][1], self.camera.global_matrix[2][1]]
                bullet_start = [
                    camera_position[0] + base_direction[0] * offset + right[0] * offset_x + up[0] * offset_y,
                    camera_position[1] + base_direction[1] * offset + right[1] * offset_x + up[1] * offset_y,
                    camera_position[2] + base_direction[2] * offset + right[2] * offset_x + up[2] * offset_y
                ]
                bullet_mesh.set_position(bullet_start)
                
                # Calcular direção aleatória dentro de um cone à frente do jogador
                # Usar ângulos aleatórios para criar variação
                angle_x = random.uniform(-0.3, 0.3)  # Variação horizontal
                angle_y = random.uniform(-0.3, 0.3)  # Variação vertical
                
                # Rotacionar a direção base usando os ângulos aleatórios
                # Primeiro rotacionar em Y (horizontal)
                cos_y = math.cos(angle_x)
                sin_y = math.sin(angle_x)
                dir_x = base_direction[0] * cos_y - base_direction[2] * sin_y
                dir_z = base_direction[0] * sin_y + base_direction[2] * cos_y
                
                # Depois rotacionar em X (vertical)
                cos_x = math.cos(angle_y)
                sin_x = math.sin(angle_y)
                dir_y = base_direction[1] * cos_x - dir_z * sin_x
                dir_z = base_direction[1] * sin_x + dir_z * cos_x
                ...
                self.scene.add(bullet_mesh)
                # Alinhar o mesh da bala com a direção do disparo
                bullet_mesh.set_direction(direction)
                bullet_mesh.scale(0.125)
                self.bullets.append({
                    "mesh": bullet_mesh,
                    "direction": direction,
                    "lifetime": 1.0,  # Tempo de vida da bala em segundos (1 segundo)
                    "is_rocket": False,  # Não é um foguete
                    "exploded": False   # Não explodiu ainda
                })
        else:
    ...
    def update_bullets(self, delta_time):
        ...
        for bullet in self.bullets:
            ...
            for character in self.character_meshes:
                ...
                collision_radius = 2.25 if bullet.get("is_rocket", False) else 1.5  # Aumentado para 1.5x o tamanho original
                ...
                if dist < collision_radius:  # Distância de colisão
                    ...
                    if random.random() < 0.25:  # 25% de chance
                        drop_pos = character_pos.copy()
                        drop_pos[1] = 1  # Definir Y para 0 (nível do chão)
                        self.create_ocarina_drop(drop_pos)
    ...
    def update_tuba_icon(self):
        if self.tuba_icon_mesh:
            self.tuba_icon_mesh.visible = self.has_tuba

    def update_guitar_icon(self):
        if self.guitar_icon_mesh:
            self.guitar_icon_mesh.visible = self.has_guitar

    def update_french_horn_icon(self):
        if self.french_horn_icon_mesh:
            self.french_horn_icon_mesh.visible = self.has_french_horn
    ...
    def update_health_display(self):
        try:
            ...
            health_width = int((self.current_health / self.max_health) * 200)
            ...
            health_text = f"{int(self.current_health)}%"
    ...
    def check_character_collision(self):
        ...
        for character in self.character_meshes:
            character_pos = np.array(character.global_position)
            dist = np.linalg.norm(player_pos - character_pos)
            
            # Se o personagem estiver muito próximo do jogador
            if dist < 1.5:  # Distância de colisão
                # Calcular dano aleatório entre 15 e 25
                damage = random.randint(15, 25)
                self.current_health = max(0, self.current_health - damage)
                self.last_damage_time = current_time
                self.update_health_display()
                print(f"Você tomou {damage}% de dano! Vida restante: {self.current_health}%")
    ...
    def update_instrument_message(self):
        ...
        show = False
        alpha = 255
        text = "Novo instrumento disponível!"  # Mensagem padrão
        ...
        if self.tuba_message_time is not None:
            ...
            if elapsed < self.tuba_message_duration:
                ...
                self.tuba_message_time = None
        elif self.french_horn_message_time is not None:
            elapsed = pygame.time.get_ticks() / 1000 - self.french_horn_message_time
            if elapsed < self.french_horn_message_duration:
                show = True
                alpha = int(255 * (1 - elapsed / self.french_horn_message_duration))
            else:
                self.french_horn_message_time = None
    ...
    def update(self):
        ...
        # Verificar tempo de aparecimento dos instrumentos
        current_time = pygame.time.get_ticks() / 1000.0
        
        # Verificar aparecimento da tuba
        if not self.tuba_has_appeared and current_time >= self.tuba_appear_time:
            self.tuba_mesh.visible = True
            self.tuba_has_appeared = True
            self.tuba_message_time = current_time
            print("Tuba apareceu!")
            
        # Verificar aparecimento do French Horn
        if not self.french_horn_has_appeared and current_time >= self.french_horn_appear_time:
            self.french_horn_mesh.visible = True
            self.french_horn_has_appeared = True
            self.french_horn_message_time = current_time
            print("French Horn apareceu!")
        ...
        if self.game_over:
            ...
            end_text = "MORRESTE!" if self.game_over_reason == "death" else "RONDA ACABOU"
            text = font_title.render(end_text, True, (255,255,255))
            rect = text.get_rect(center=(w//2, int(h*0.3)))
            self.end_hud_surface.blit(text, rect)
            ...
            kills_text = f"Total de Kills: {self.kill_count}"
            kills_surface = font_kills.render(kills_text, True, (255,255,255))
            kills_rect = kills_surface.get_rect(center=(w//2, int(h*0.4)))
            self.end_hud_surface.blit(kills_surface, kills_rect)
            
            # Botões com novo estilo
            font_btn = pygame.font.Font(None, 36)
            btn_w, btn_h = 200, 50
            btn1_center = (w * 0.35, h * 0.55)
            btn2_center = (w * 0.65, h * 0.55)
            ...
            mx, my = pygame.mouse.get_pos()
            screen_w, screen_h = self._screen.get_size()
            btn1_center_screen = (screen_w * 0.35, screen_h * 0.55)
            btn2_center_screen = (screen_w * 0.65, screen_h * 0.55)
            
            btn1_rect = pygame.Rect(0, 0, btn_w, btn_h)
            btn1_rect.center = btn1_center_screen
            btn2_rect = pygame.Rect(0, 0, btn_w, btn_h)
            btn2_rect.center = btn2_center_screen
            
            # Botão Reiniciar
            color1 = btn_hover_color if btn1_rect.collidepoint(mx, my) else btn_color
            pygame.draw.rect(self.end_hud_surface, color1, pygame.Rect(0, 0, btn_w, btn_h).move(btn1_center[0]-btn_w//2, btn1_center[1]-btn_h//2), border_radius=20)
            txt1 = font_btn.render("Reiniciar", True, (255,255,255))
            txt1_rect = txt1.get_rect(center=btn1_center)
            self.end_hud_surface.blit(txt1, txt1_rect)
            
            # Botão Sair
            color2 = btn_hover_color if btn2_rect.collidepoint(mx, my) else btn_color
            pygame.draw.rect(self.end_hud_surface, color2, pygame.Rect(0, 0, btn_w, btn_h).move(btn2_center[0]-btn_w//2, btn2_center[1]-btn_h//2), border_radius=20)
            txt2 = font_btn.render("Sair", True, (255,255,255))
            txt2_rect = txt2.get_rect(center=btn2_center)
            self.end_hud_surface.blit(txt2, txt2_rect)
            ...
            if not self.game_over:
                ...
                if not self.paused:
                    ...
                    # Mover personagens automaticamente em direção ao jogador
                    player_pos = np.array(self.rig.global_position)
                    for character in self.character_meshes:
                        old_pos = np.array(character.global_position)
                        # Calcular direção para o jogador, mas ignorar o eixo Y
                        player_pos_flat = player_pos.copy()
                        player_pos_flat[1] = old_pos[1]
                        direction = player_pos_flat - old_pos
                        ...
                        if distance > 0:
                            ...
                            # Adicionar animação de flutuação vertical
                            float_offset = math.sin(self.animation_time * self.float_speed) * self.float_amplitude
                            new_pos[1] = 2.0 + float_offset  # Altura base de 2.0 + flutuação
                            
                            character.set_position(new_pos.tolist())
                            
                            # Calcular direção direta para o jogador (incluindo altura)
                            look_direction = player_pos - np.array(character.global_position)
                            look_direction[1] = 0  # Ignorar diferença de altura para rotação Y
                            
                            # Calcular ângulo para olhar para o jogador (invertido)
                            angle = math.atan2(-look_direction[0], -look_direction[2])
                            angle_degrees = math.degrees(angle)
                            
                            # Resetar a matriz de transformação
                            character.local_matrix = Matrix.make_identity()
                            
                            # Aplicar rotações na ordem correta
                            character.rotate_y(math.radians(angle_degrees)+3.14)  # Rotação para olhar para o jogador
                            # Removida a rotação X que fazia olhar para baixo
                    ...
                    if self.tuba_mesh.visible or self.guitar_mesh.visible or self.french_horn_mesh.visible:
                        ...
                        # Atualizar posição da tuba se estiver visível
                        ...
                        # Atualizar posição da guitarra se estiver visível
                        if self.guitar_mesh.visible:
                        ...
                        # Atualizar posição do French Horn se estiver visível
                        if self.french_horn_mesh.visible:
                            new_y = self.french_horn_initial_position[1] + float_offset
                            self.french_horn_mesh.set_position([
                                self.french_horn_initial_position[0],
                                new_y,
                                self.french_horn_initial_position[2]
                            ])
                            
                            # Rotacionar o French Horn em torno do eixo Y
                            self.french_horn_mesh.rotate_y(self.rotation_speed * self.delta_time)
                    
                    # Verificar colisão com a tuba
                    if not self.has_tuba and not self.has_guitar and not self.has_french_horn:  # Só pode pegar a tuba se não tiver nenhum instrumento
                    ...
                    if not self.has_guitar and not self.has_tuba and not self.has_french_horn:  # Só pode pegar a guitarra se não tiver nenhum instrumento
                    ...
                    # Verificar colisão com o French Horn
                    if not self.has_french_horn and not self.has_tuba and not self.has_guitar:  # Só pode pegar se não tiver nenhum instrumento
                        french_horn_pos = np.array(self.french_horn_mesh.global_position)
                        player_pos = np.array(self.rig.global_position)
                        dist = np.linalg.norm(french_horn_pos - player_pos)
                        now = pygame.time.get_ticks() / 1000
                        can_pickup = (
                            self.french_horn_drop_time is None or
                            (now - self.french_horn_drop_time) >= 2.0
                        )
                        if dist < 1.0 and can_pickup:
                            self.has_french_horn = True
                            self.french_horn_mesh.visible = False
                            self.french_horn_pickup_time = now
                            print('French Horn apanhado!')
                    ...
                    # Largar a guitarra ao pressionar G
                    ...
                    # Largar o French Horn ao pressionar G
                    if self.has_french_horn and keys[pygame.K_g]:
                        self.has_french_horn = False
                        self.french_horn_weapon_mesh.visible = False
                        ...
                        # Posicionar o French Horn 1 unidade à frente do jogador
                        drop_pos = player_pos + camera_direction * 1.0
                        drop_pos[1] = 1.0  # Altura fixa
                        
                        # Atualizar a posição inicial do French Horn para a nova posição
                        self.french_horn_initial_position = drop_pos.tolist()
                        
                        # Resetar o French Horn
                        self.french_horn_mesh.local_matrix = Matrix.make_identity()
                        self.french_horn_mesh.set_position(drop_pos.tolist())
                        self.french_horn_mesh.scale(0.2)
                        self.french_horn_mesh.rotate_x(-math.pi/2)  # Restaurar orientação
                        
                        # Calcular ângulo em Y baseado na direção da câmera
                        y_angle = math.atan2(camera_direction[0], camera_direction[2])
                        self.french_horn_mesh.rotate_y(y_angle)
                        
                        # Resetar o tempo de animação para começar do zero
                        self.animation_time = 0
                        
                        self.french_horn_mesh.visible = True
                        self.french_horn_drop_time = pygame.time.get_ticks() / 1000
                        print("French Horn largado!")
                    
                    # Só permitir disparar se tiver a tuba, a guitarra ou o French Horn
                    if self.has_tuba or self.has_guitar or self.has_french_horn:
                        ...
                        if mouse_buttons[0]:  # Se o botão do mouse está pressionado
                            if self.has_tuba and (current_time - self.last_shot_time) >= self.shot_cooldown:
                                ...
                                self.create_bullet()
                                self.last_guitar_shot_time = current_time
                            elif self.has_french_horn and (current_time - self.last_shot_time) >= self.shot_cooldown:
                                self.create_bullet()
                                self.last_shot_time = current_time
                    ...
                    # Atualizar posição e visibilidade da luz baseado nos instrumentos
                    if self.tuba_mesh.visible:
                        # Se a tuba está visível, mover a luz para ela
                        tuba_pos = self.tuba_mesh.global_position
                        self.tuba_light.set_position([tuba_pos[0], 0.5, tuba_pos[2]])
                        self.tuba_light._color = [
                            base_color[0] * intensity,
                            base_color[1] * intensity,
                            base_color[2] * intensity
                        ]
                    elif self.guitar_mesh.visible:
                        # Se a guitarra está visível, mover a luz para ela
                        guitar_pos = self.guitar_mesh.global_position
                        self.tuba_light.set_position([guitar_pos[0], 0.5, guitar_pos[2]])
                        self.tuba_light._color = [
                            base_color[0] * intensity,
                            base_color[1] * intensity,
                            base_color[2] * intensity
                        ...
                        # Se nenhum instrumento está visível, desligar a luz
                        self.tuba_light._color = [0, 0, 0]

                    self.update_tuba_icon()
                    self.update_guitar_icon()
                    self.update_french_horn_icon()
                    ...
                    # Mostrar o French Horn como arma (em frente à câmara)
                    self.french_horn_weapon_mesh.visible = self.has_french_horn
                ...
                # Verificar se é hora de fazer a tuba aparecer
                if not self.tuba_has_appeared and self.total_time - self.remaining_time >= self.tuba_appear_time:
                    self.tuba_has_appeared = True
                    self.tuba_mesh.visible = True
                    self.tuba_message_time = pygame.time.get_ticks() / 1000  # Tempo atual em segundos
                    print("Tuba apareceu no centro do mapa!")
                
                # Atualizar a mensagem de instrumento disponível
                self.update_instrument_message()
    ...
    def restart_game(self):
        ...
        self.french_horn_drop_time = None
        self.last_shot_time = 0
        self.last_guitar_shot_time = 0
        self.animation_time = 0
        
        # Resetar posição do jogador
        self.rig.set_position([0, 1, 0])
        self.camera.set_position([0, 1, 0])
        
        # Resetar posição da tuba
        self.tuba_mesh.set_position(self.tuba_initial_position)
        self.tuba_mesh.visible = True
        self.tuba_weapon_mesh.visible = False
        
        # Resetar posição da guitarra
        self.guitar_mesh.set_position(self.guitar_initial_position)
        self.guitar_mesh.visible = True
        self.guitar_weapon_mesh.visible = False

        # Resetar posição do French Horn
        self.french_horn_mesh.set_position(self.french_horn_initial_position)
        self.french_horn_mesh.visible = True
        self.french_horn_weapon_mesh.visible = False
...
def show_main_menu():
    ...
    pygame.display.set_caption("Music Maniac - Menu")

    # Carregar frames do GIF (pygame não suporta GIF animado nativamente)
    # Vamos carregar só o primeiro frame para simplicidade
    try:
        bg_image = pygame.image.load("images/hell.gif")
    ...
    btn_rect.center = (screen_size[0] // 2, int(screen_size[1] * 0.7))
    ...
    while running:
        ...
        # Desenhar título
        title_surf = font_title.render("Music Maniac", True, (0, 0, 0))
        title_rect = title_surf.get_rect(center=(screen_size[0] // 2, int(screen_size[1] * 0.3)))
        ...
        btn_text = font_btn.render("Começar", True, (255, 255, 255))
